...
function rcbrowserHeight() {
  ...
  var _maxHeight = document.documentElement.clientHeight * 0.9
...
$(document).on('change', 'select.arbitrator', function() {
  ...
  var tos_section = $(this)
    .closest('.select-container')
    .find('div.arbitrator-tos')
  if (tos_url) {
...
$('#your-question-answer-window .rcbrowser__close-button').on('click', function(
  e
) {
  e.preventDefault()
  e.stopPropagation()
...
$(document).on(
  ...
  function(e) {
    ...
    getAccount().then(function() {
      ...
      var expected_question_fee_attr = win
        .find('.arbitrator option:selected')
        .attr('data-question-fee')
      var expected_question_fee = expected_question_fee_attr
        ? new BigNumber(expected_question_fee_attr)
        : new BigNumber(0)
      ...
      if (validate(win)) {
        ...
        var question_id = rc_question.questionID(
          template_id,
          qtext,
          arbitrator,
          timeout_val,
        ...
        console.log(
          'content_hash inputs for content hash ',
          rc_question.contentHash(template_id, opening_ts, qtext),
          template_id,
          opening_ts,
          qtext
        )
          ...
          rc.arbitrator_question_fees.call(arbitrator).then(function(fee) {
            ...
            rc.askQuestion
              ...
              .then(function(txid) {
                ...
                ;(fake_call[Qi_content_hash - 1] = rc_question.contentHash(
                  template_id,
                  parseInt(opening_ts),
                  qtext
                )),
...
$(document).on('click', '.answer-claim-button', function() {
  ...
  var doClaim = function(is_single_question, question_detail) {
    ...
    rc.claimMultipleAndWithdrawBalance
      .sendTransaction(
...
function handlePotentialUserAction(entry, is_watch) {
  ...
  if (
    !q_min_activity_blocks[question_id] ||
    entry.blockNumber < q_min_activity_blocks[question_id]
  ) {
  ...
  if (
    ...
    submitted_question_id_timestamp[question_id] > 0
  ) {
...
function updateClaimableDisplay() {
  ...
  if (claiming.total.gt(0)) {
    ...
    $('.answer-claiming-container')
      .find('.claimable-eth')
      .text(web3js.fromWei(claiming.total.toNumber(), 'ether'))
  ...
  rc.balanceOf.call(account).then(function(result) {
    ...
    if (ttl.gt(0)) {
      $('.answer-claim-button.claim-all')
        .find('.claimable-eth')
        .text(web3js.fromWei(ttl.toNumber(), 'ether'))
...
function scheduleFinalizationDisplayUpdate(question) {
  ...
  if (
    ...
    if (question_event_times[question_id]) {
      if (
        question_event_times[question_id].finalization_ts ==
        question[Qi_finalization_ts]
      ) {
    ...
    if (!is_done) {
      ...
      var update_time =
        1000 +
        question[Qi_finalization_ts].toNumber() * 1000 -
        new Date().getTime()
...
function _ensureAnswerRevealsFetched(
  question_id,
  freshness,
  start_block,
  question
  ...
  for (var i = 0; i < question['history'].length; i++) {
    if (question['history'][i].args['is_commitment']) {
      if (!question['history'][i].args['revealed_block']) {
        ...
        console.log(
          '_ensureAnswerRevealsFetched found commitment, block',
          earliest_block,
          'bond',
          bond
        )
  ...
  if (earliest_block > 0) {
    return new Promise((resolve, reject) => {
      var reveal_logs = rc.LogAnswerReveal(
        { question_id: question_id },
        { fromBlock: earliest_block, toBlock: 'latest' }
      )
      reveal_logs.get(function(error, answer_arr) {
        if (error) {
          ...
          for (var j = 0; j < answer_arr.length; j++) {
            ...
            question['history'][idx].args['revealed_block'] =
              answer_arr[j].blockNumber
            question['history'][idx].args['answer'] =
              answer_arr[j].args['answer']

            var commitment_id = rc_question.commitmentID(
              question_id,
  ...
  switch (data_type) {
    ...
    case 'question_call':
      ...
      if (data && freshness >= question.freshness.question_call) {
        ...
        question[Qi_is_pending_arbitration] =
          data[Qi_is_pending_arbitration - 1]
    ...
    if (isDataFreshEnough(question_id, 'question_log', freshness)) {
      ...
      question_logs.get(function(error, question_arr) {
        if (error || question_arr.length != 1) {
          ...
          var question = filledQuestionDetail(
            question_id,
            'question_log',
            called_block,
            question_arr[0]
...
function _ensureQuestionTemplateFetched(
  question_id,
  template_id,
  qtext,
  freshness
...
function _ensureAnswersFetched(
  question_id,
  freshness,
  start_block,
  injected_data
...
function ensureQuestionDetailFetched(
  question_id,
  ql,
  qi,
  qc,
  ...
  return new Promise((resolve, reject) => {
    _ensureQuestionLogFetched(question_id, ql)
      .then(function(q) {
        return _ensureQuestionDataFetched(question_id, qc)
      })
      .then(function(q) {
        return _ensureQuestionTemplateFetched(
          question_id,
          q[Qi_template_id],
          q[Qi_question_text],
          qi
      ...
      .then(function(q) {
        return _ensureAnswersFetched(
          question_id,
          al,
          q[Qi_question_created_block],
          injected_data
        )
      })
      .then(function(q) {
...
function populateSection(section_name, question_data, before_item) {
  ...
  if (web3js.fromWei(question_data[Qi_bounty], 'ether') < 0.01) {
    balloon_html +=
      'The reward is very low.<br /><br />This means there may not be enough incentive to enter the correct answer and back it up with a bond.<br /><br />'
  }
...
function updateSectionEntryDisplay(question) {
  $(
    'div.questions__item[data-question-id="' + question[Qi_question_id] + '"]'
  ).each(function() {
...
function populateSectionEntry(entry, question_data) {
  ...
  if (isAnswered(question_data)) {
    entry
      ...
      .text(rc_question.getAnswerString(question_json, best_answer))
  ...
  if (isArbitrationPending(question_data)) {
    ...
    if (is_answered) {
      entry
        ...
        .attr(
          'datetime',
          rc_question.convertTsToString(question_data[Qi_finalization_ts])
        )
      ...
      entry
        ...
        .attr(
          'datetime',
          rc_question.convertTsToString(question_data[Qi_creation_ts])
        )
...
function depopulateSection(section_name, question_id) {
  ...
  if (item.length) {
    ...
    var current_last_qid = section
      .find('.questions__item')
      .last()
      .attr('data-question-id')
    var current_last_idx = display_entries[section_name]['ids'].indexOf(
      current_last_qid
    )
...
function handleQuestionLog(item) {
  ...
  var question_data = filledQuestionDetail(
    question_id,
    'question_log',
    item.blockNumber,
    item
  ...
  ensureQuestionDetailFetched(question_id, 1, 1, item.blockNumber, -1).then(
    function(question_data) {
      ...
      if (is_finalized) {
        var insert_before = update_ranking_data(
          'questions-resolved',
          question_id,
          question_data[Qi_finalization_ts],
          'desc'
        )
        if (insert_before !== -1) {
        ...
        if (insert_before !== -1) {
          ...
          $('#questions-latest')
            ...
            .css('display', 'none')
          if (
            display_entries['questions-latest']['ids'].length > 3 &&
            $('#questions-latest')
              .find('.loadmore-button')
              .css('display') == 'none'
          ) {
        ...
        if (insert_before !== -1) {
          ...
          $('#questions-high-reward')
            ...
            .css('display', 'none')
          if (
            display_entries['questions-high-reward']['ids'].length > 3 &&
            $('#questions-high-reward')
              .find('.loadmore-button')
              .css('display') == 'none'
          ) {
...
function openQuestionWindow(question_id) {
  ...
  ensureQuestionDetailFetched(question_id, 1, 1, 1, -1).then(function(
    question
  ) {
    displayQuestionDetail(question)
...
function displayQuestionDetail(question_detail) {
  ...
  if (
    rcqa.find('[name="input-answer"]').hasClass('rcbrowser-input--date--answer')
  ) {
...
function populateQuestionWindow(rcqa, question_detail, is_refresh) {
  ...
  rcqa
    .find('.reward-value')
    .text(web3js.fromWei(question_detail[Qi_bounty], 'ether'))
  ...
  if (isAnswerActivityStarted(question_detail)) {
    ...
    if (isAnswered(question_detail)) {
      ...
      var label = rc_question.getAnswerString(
        question_json,
        question_detail[Qi_best_answer]
      )
      current_container
    ...
    if (question_detail['history'].length) {
      ...
      for (var i = 0; i < idx; i++) {
        ...
        var hist_id =
          ...
          web3js.sha3(question_id + ans.answer + ans.bond.toString())
        ...
        hist_item
          .find('.answer-bond-value')
          .text(web3js.fromWei(ans.bond.toNumber(), 'ether'))
        hist_item
  ...
  rcqa
    .find('.bond-value')
    .text(web3js.fromWei(question_detail[Qi_bond], 'ether'))
  ...
  window.setTimeout(function() {
    rcqa
      ...
      .attr('data-last-seen-bond', '0x' + question_detail[Qi_bond].toString(16))
  }, 2000)
  ...
  balloon
    ...
    .text(rc_question.secondsTodHms(question_detail[Qi_timeout]))
  balloon
    .find('.setting-info-content-hash')
    .text(question_detail[Qi_content_hash])
  balloon
    .find('.setting-info-question-id')
    .text(question_detail[Qi_question_id])
  ...
  if (question_detail['history_unconfirmed'].length) {
    ...
    ans_data
      .find('.answer-bond-value')
      .text(web3js.fromWei(unconfirmed_answer.bond.toNumber(), 'ether'))
  ...
  if (!isArbitrationPending(question_detail) && !isFinalized(question_detail)) {
    Arbitrator.at(question_detail[Qi_arbitrator])
      .then(function(arb) {
      ...
      .then(function(fee) {
        ...
        rcqa
          ...
          .text(web3js.fromWei(fee.toNumber(), 'ether'))
  ...
  if (!is_refresh) {
    // answer form
    var ans_frm = makeSelectAnswerInput(
      question_json,
      question_detail[Qi_opening_ts].toNumber()
    )
  ...
  if (isFinalized(question_detail)) {
    ...
    if (tot.toNumber() == 0) {
      ...
      rcqa
        ...
        .text(web3js.fromWei(tot.toNumber(), 'ether'))
    }
  } else {
    rcqa.removeClass('is-claimable')
  }
...
function renderNotifications(qdata, entry) {
  ...
  switch (evt) {
    case 'LogNewQuestion':
      var notification_id = web3js.sha3(
        'LogNewQuestion' +
          entry.args.question_text +
          entry.args.arbitrator +
          entry.args.timeout.toString()
      )
    ...
    case 'LogFundAnswerBounty':
      var notification_id = web3js.sha3(
        'LogFundAnswerBounty' +
          entry.args.question_id +
          entry.args.bounty.toString() +
          entry.args.bounty_added.toString() +
          entry.args.user
      )
      if (entry.args.user == account) {
...
function insertQAItem(
  ...
  question_section
    .find('.your-qa__questions__item[data-question-id=' + question_id + ']')
    .remove()
...
function renderQAItemAnswer(
  ...
  var question_section = $('#your-question-answer-window').find(
    '.your-qa__questions'
  )
  var answer_section = $('#your-question-answer-window').find(
    '.your-qa__answers'
  )
  ...
  if (isAnsweredOrAnswerActive(question)) {
    if (isFinalized(question)) {
      ...
      question_window
        ...
        .attr(
          'datetime',
          rc_question.convertTsToString(question[Qi_finalization_ts])
        )
...
function pushWatchedAnswer(answer) {
  ...
  for (var i = 0; i < length; i++) {
    if (
      question_detail_list[question_id]['history'][i].args.answer ==
      answer.args.answer
    ) {
      already_exists = true
      break
      ...
      .then(function(current_question) {
        ...
        switch (question_json['type']) {
          case 'bool':
            ...
            if (
              ans.isNaN() ||
              !(
                ans.equals(new BigNumber(0)) ||
                ans.equals(new BigNumber(1)) ||
                ans.equals(new BigNumber(invalid_value))
              )
            ) {
              parent_div
          ...
          case 'uint':
            ...
            if (ans.isNaN()) {
              ...
              !ans.equals(new BigNumber(invalid_value)) &&
              (ans.lt(rc_question.minNumber(question_json)) ||
                ans.gt(rc_question.maxNumber(question_json)))
            ) {
              err = true
        ...
        if (bond.lt(min_amount)) {
          ...
          parent_div
            .find('div.input-container.input-container--bond')
            .find('.min-amount')
            .text(web3js.fromWei(min_amount, 'ether'))
          is_err = true
        }
        ...
        if (USE_COMMIT_REVEAL) {
          ...
          var nonce = nonceFromSeed(
            web3.sha3(question_id + answer_plaintext + bond)
          )
          ...
          return rc
            .submitAnswerCommitment(
              question_id,
              answer_hash,
              current_question[Qi_bond],
              account,
              { from: account, gas: 200000, value: bond }
            )
            .then(function(txid) {
              console.log('got submitAnswerCommitment txid', txid)
              return rc.submitAnswerReveal.sendTransaction(
                question_id,
                answer_plaintext,
                nonce,
                bond,
          ...
          return rc.submitAnswer.sendTransaction(
            question_id,
            new_answer,
            current_question[Qi_bond],
            {
      ...
      .then(function(txid) {
        ...
        ensureQuestionDetailFetched(
          ...
          .catch(function() {
            ...
            ensureQuestionDetailFetched(question_id, 0, 0, 0, -1).then(function(
              question
            ) {
              updateQuestionWindowIfOpen(question)
            })
    ...
    Arbitrator.at(question_detail[Qi_arbitrator])
      ...
      .then(function(fee) {
        ...
        arbitrator
          .requestArbitration(
            question_id,
            new BigNumber(last_seen_bond_hex, 16),
...
function show_bond_payments(ctrl) {
  ...
  ensureQuestionDetailFetched(question_id)
    .then(function(question) {
      ...
      if (existing_answers[new_answer]) {
        ...
        if (existing_answers[new_answer].args.user == account) {
          ...
          frm
            ...
            .text(web3js.fromWei(payable, 'ether'))
        } else {
          frm
...
$(document).on('keyup', '.rcbrowser-input.rcbrowser-input--number', function(
  e
) {
  ...
  let bond_validation = function(ctrl) {
    ...
    if (current_idx >= 0) {
      current_bond =
        question_detail_list[question_id]['history'][current_idx].args.bond
    }
...
$(document).on('click', '.invalid-switch-container a.valid-text-link', function(
  evt
) {
  evt.stopPropagation()
  var inp = $(this)
...
function updateRankingSections(question, changed_field, changed_val) {
  ...
  if (changed_field == Qi_bounty || changed_field == Qi_finalization_ts) {
    var insert_before = update_ranking_data(
      'questions-high-reward',
      question_id,
      question[Qi_bounty].plus(question[Qi_bond]),
      'desc'
    )
...
function pageInit(account) {
  ...
  RealityCheckRealitio.setProvider(
    new Web3.providers.HttpProvider(RPC_NODES[network_id])
  )
...
function reflectDisplayEntryChanges() {
  ...
  for (var i = 0; i < detypes.length; i++) {
    ...
    var has_items =
      $('#' + detype)
        .find('div.questions-list div.questions__item')
        .size() > 0
    if (has_items) {
      $('#' + detype)
...
function fetchUserEventsAndHandle(filter, start_block, end_block) {
  ...
  var answer_revealed = rc.LogAnswerReveal(filter, {
    fromBlock: start_block,
    toBlock: end_block
  })
...
function getAccount(fail_soft) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (typeof ethereum === 'undefined') {
      if (!fail_soft) {
        $('body')
          .addClass('error-no-metamask-plugin')
          .addClass('error')
      }
      reject(
...
window.addEventListener('load', function() {
  ...
  web3realitio = new Web3(
    new Web3.providers.HttpProvider('https://rc-dev-3.socialminds.jp')
  )
  ...
  web3js.version.getNetwork((err, net_id) => {
    if (err === null) {
      if (!initNetwork(net_id)) {
        $('body')
          .addClass('error-invalid-network')
          .addClass('error')
        return
      }
    ...
    web3js.eth.getBlock('latest', function(err, result) {
      ...
      RealityCheck.deployed().then(function(instance) {
        ...
        if (args['question']) {
          ...
          ensureQuestionDetailFetched(args['question']).then(function(
            question
          ) {
          ...
          RealitioArbitratorProxy.at(
            {
              1: '0x15daedbebf8083620ffbe9a675fb53b40c12d2ac',
              42: '0xfa74dee72c7279e420f65e09297b21e196d1db75'
            }[net_id]
  ...
  if (args['category']) {
    $('#filter-list')
      .find("[data-category='" + args['category'] + "']")
      .addClass('selected')
  } else {
    $('#filter-list')
      .find("[data-category='all']")
      .addClass('selected')
  }
...
$('.continue-read-only-message').click(function(e) {
  ...
  e.stopPropagation()
  $('body')
    .removeClass('error-no-metamask-plugin')
    .removeClass('error')
})
